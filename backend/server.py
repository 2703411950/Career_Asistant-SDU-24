...
url3 = 'http://127.0.0.1:7861/chat/knowledge_base_chat'
...
def upload_file():
    ...
    question = f"你是{company}公司的面试官，我要面试{job}岗位，现在要进行{interview},这是我的简历相关信息{extracted},\n\n请你现在问5个面试问题，每个问题以问号结尾，确保每个问题占一行"
    result = [f"你好，我是{company}公司的面试官，下面我们将开始面试，你准备好了嘛？"]
    result2 = Turing(question).split('\n')
    while len(result2) < 5:
        result2 = Turing(question).split('\n')
    result += result2
    return jsonify({
...
def showExperience():
    ...
    try:
        ...
        result = sql.execute(
            f"SELECT title,content FROM goodexperience WHERE company='{company}' AND ({job_conditions}) LIMIT {offset}, 1")
        print(result)
    except Exception as e:
